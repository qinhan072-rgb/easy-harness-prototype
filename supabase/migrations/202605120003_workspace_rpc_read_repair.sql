...
-- Workspace read repair.
-- The app reads request/order workspaces through explicit RPCs so customer,
-- staff, normal-window, and incognito sessions all receive the same server-owned
-- records. These functions authorize by auth.uid() and profile role, then return
-- nested rows shaped for the existing frontend model.
...
begin
  ...
  select coalesce(role in ('staff', 'admin') and status = 'active', false)
  into v_is_staff
